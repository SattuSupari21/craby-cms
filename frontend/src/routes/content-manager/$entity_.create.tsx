...
function CreateNewEntryComponent() {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            if (!res.ok) toast({ title: "Error", description: "An error has occurred" })
            const data = await res.json()
            if (data.error) toast({ title: "Error", description: "An error has occurred" })

            toast({ title: "Success", description: "Added new entry!" })
